...
let load = async function(){
    ...
    for(let one of ones){
        ...
        res[one.name]=[persons.rows[0],persons.rows[1]]
        ...
        for(let two of twos){
            ...
            res[one.name][two.name]=[persons.rows[0],persons.rows[1]]
            ...
            for(let three of threes){
                ...
                res[one.name][two.name][three.name]=[persons.rows[0],persons.rows[1]]
            }
        }
    }
    console.log(res)
...
// load()
getTreeData('nFYAAAAAG+zM567U').then((v)=>console.log(v))